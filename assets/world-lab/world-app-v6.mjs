...
function selectHistoryPlace(id,reveal=false){
 ...
 if(id>=0&&humanHistory){const site=humanHistory.sites[id];if(site&&reveal){const x=world.mesh.x[site.nodeId],z=world.mesh.z[site.nodeId];if(x<box.x||z<box.z||x>box.x+box.size||z>box.z+box.size){parentView=true;updateURL({parent:1});applyWindow(true);}}historyControls.openPlace();}
 renderHistory();paint();
}
...
function syncControls(){const reference=$('worldSource').value!=='generated';for(const id of ['seed','newSeed','continentCount','crustScale','size','relief'])$(id).disabled=reference;$('newWindow').hidden=reference;$('parentMap').hidden=reference;$('climate').disabled=!reference;document.querySelector('[data-stage="5"]').disabled=reference;
...
function applyWindow(reset=false){if(!world)return;box=currentBox();view.setWorld(world,box);if(reset)view.reset();$('parentMap').textContent=parentView?'Window':'Parent';
 ...
 renderHistory();paint();if(selected>=0)inspect(selected,false);
}
function showStage(value){
 stage=clamp(value|0,1,snapshots[4]&&!snapshots[4].parentDomain?4:5);if(stage!==5)historyControls.stop();
 document.body.dataset.stage=String(stage);document.querySelectorAll('[data-stage]').forEach(b=>b.setAttribute('aria-pressed',String(Number(b.dataset.stage)===stage)));$('progress').style.width=`${stage/5*100}%`;updateURL({stage});
 $('stageDescription').textContent=['','Terrain before lakes or people.','Water budget and drainage solved on the complete input domain.','Environments use modeled water and the selected climate forcing.','Geographic potential for food, movement, and settlement.','Settlements and their countryside, one generation at a time.'][stage];
 renderHistory();
...
function paint(){if(!world||!box)return;const w=world,N=w.height.length,kind=actualMode(),historyOverlays=historyControls.overlays(),colors=new Uint8ClampedArray(N*4),normal=!view.gl?meshNormals(w.mesh,Float32Array.from(view.modelHeights,h=>h/1000*view.exag)):null,localColors=stage===5?new Uint8ClampedArray(N*4):null,localNormals=stage===5&&view.gl?meshNormals(w.mesh,Float32Array.from(view.modelHeights,h=>h/1000*view.exag)):normal;
 ...
 if(stage===5&&kind==='natural')drawHumanHistory(ctx,w,humanHistory,historyFrame(),box,{...historyOverlays,selectedSite,symbolScale:innerWidth<700?2.5:1.5});
 if(parentView&&w.parentDomain&&stage!==5){const selectedBox=chooseWindow(w,cropIndex),[x,z]=xy(selectedBox.x,selectedBox.z);ctx.strokeStyle='#ffe0a1';ctx.lineWidth=5;ctx.strokeRect(x,z,selectedBox.size*scale,selectedBox.size*scale);}
...
function inspect(id,repaint=true){if(!world||id<0||id>=world.height.length)return;
 if(stage===5&&historyFrame()){const nearest=nearestHistorySite(world,humanHistory,historyFrame(),id,box,Math.max(world.stepKm*1.5,box.size*.035));if(nearest>=0){selectHistoryPlace(nearest);return;}selectedSite=-1;renderHistory();}
 selected=id;const w=world,b=stage>=2?w.basinWater?.[w.potentialBasinId?.[id]]:null,facts=[['Elevation',fmt(w.height[id])+' m'],['Physical spacing',fmt(w.stepKm,2)+' km']];
...
function watch(){if(playing){stop();return;}playing=true;$('play').textContent='Ⅱ Pause';let s=1;const advance=()=>{if(!playing)return;if(!snapshots[s]){timer=setTimeout(advance,150);return;}showStage(s);if(s===(snapshots[4]?.parentDomain?5:4)){stop();return;}s++;timer=setTimeout(advance,1900);};advance();}
...
$('worldSource').onchange=()=>{syncControls();stage=$('worldSource').value==='generated'?5:4;generate();};$('regenerate').onclick=generate;$('newSeed').onclick=()=>{$('seed').value=crypto.getRandomValues(new Uint32Array(1))[0];cropIndex=0;generate();};
$('newWindow').onclick=()=>{cropIndex++;parentView=false;selected=-1;selectedSite=-1;$('inspect').hidden=true;updateURL({crop:cropIndex,parent:0});applyWindow(true);};$('parentMap').onclick=()=>{parentView=!parentView;updateURL({parent:parentView?1:0});applyWindow(true);};
