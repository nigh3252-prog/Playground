import {clamp,random32,noise} from './world-core.mjs';
...
function weightedType(r){
  const x=r();
  if(x<.30)return 1;      // glacial
  if(x<.49)return 2;      // rift
  if(x<.70)return 3;      // volcanic
  return 4;               // old river country
}
function distance2(a,b){return(a.x-b.x)**2+(a.z-b.z)**2;}

/** Build several geological provinces automatically from the seed.
 * The result is metadata + feature recipes. It does not alter the mesh.
 * `baseAt` returns pre-history terrain so volcanic centers can prefer high land.
 */
export function planGeology({seed,sizeKm,northAxis,eastAxis,baseAt}){
  ...
  function glacialProvince(){
    ...
    for(let k=0;k<count;k++){
      ...
      const dx=end.x-start.x,dz=end.z-start.z,normal={x:-dz,z:dx},L=Math.hypot(dx,dz),bend=(r()-.5)*.10*sizeKm;
      const points=[start,
        {x:mix(start.x,end.x,.30)+normal.x/L*bend,z:mix(start.z,end.z,.30)+normal.z/L*bend},
        {x:mix(start.x,end.x,.64)-normal.x/L*bend*.45,z:mix(start.z,end.z,.64)-normal.z/L*bend*.45},end];
...
/** Apply overlapping histories to base elevation. Returns actual modified land.
 * Boundaries are intentionally soft/noisy: features are processes, not stamps.
 */
export function applyGeology(plan,x,z,baseHeight){
  ...
  for(const f of plan.features){let influence=0;
    if(f.type===1){
      ...
      const overdeep=f.overdeepM*Math.exp(-(((q.t-.68)/.15)**2)),carve=(120+210*q.t+overdeep)*influence;
      const sill=f.sillM*Math.exp(-(((q.t-.91)/.055)**2))*Math.exp(-((d/.85)**4));land+=-carve+sill;
    }else if(f.type===2){
      ...
      let r=Math.hypot(a,b);r*=1+.10*noise(x/f.radiusKm,z/f.radiusKm,f.salt)+.045*Math.sin(theta*5+f.offsetAngle);
      influence=Math.exp(-((r/1.48)**4));
      const edifice=f.upliftM*Math.exp(-((r/.92)**2)),rim=f.rimM*Math.exp(-(((r-.35)/.105)**2)),collapse=f.collapseM*Math.exp(-((r/.225)**4));
