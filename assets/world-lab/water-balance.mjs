...
export const WATER_STATES=[['Land','#879678'],['Permanent lake','#397b92'],['Seasonally wet basin','#769681'],['Dry / leaky basin','#c2a27b'],['Mapped reference lake','#467fac'],['Ocean','#244859']];
...
export function solveBasin(samples,spill,inflow,retention){
 ...
 const estimate=level=>{let supply=inflow,wetArea=0,evap=0,seep=0,precip=0,landRunoff=0;
  ...
  // Unsaturated leakage must not exceed available supply. The residual drives
  // lake rise/overflow only after this floor loss has been supplied.
...
/** Route only within a depression to its lowest floor (closed) or outlet
 * (overflowing). Priority ordering avoids loops on flat water surfaces.
 */
...
export function resolveSurfaceWater(potential,climate){
 ...
 for(let k=compOrder.length-1;k>=0;k--){const c=compOrder[k];let amount,target=-1;
  if(c<N){if(w.ocean[c]||observed[c])continue;amount=supply[c]+local[c];target=receiver[c]>=0?comp[receiver[c]]:-1;}
   ...
   const b=solveBasin(samples,w.lakeBodies[g].level,supply[c],retentionFor(w.lakeBodies[g].history,w.config.seed+floor[g]*19));b.id=g;b.history=w.lakeBodies[g].history;b.root=b.overflowing?first[g]:floor[g];budgets[g]=b;
   routeBasin(w,members,b.root,b.overflowing?exit[g]:-1,receiver);
 ...
 for(let g=0;g<groups.length;g++){const b=budgets[g];if(!b)continue;const id=lakeBodies.length;let visible=0;
  for(const i of groups[g]){waterState[i]=b.status==='dry'?3:b.status==='seasonal'?2:0;
   if(b.status!=='dry'&&b.status!=='seasonal'&&height[i]<b.level-.75){lake[i]=1;lakeId[i]=id;waterSurface[i]=b.level;waterState[i]=1;visible++;}}
 ...
 // Recomputed order has all closed sinks, not the old forced spill-to-sea flow.
 ...
 return{...w,stage:2,version:'regional-world-v5',potentialSpill:w.filled,potentialReceiver:w.receiver,potentialBasinId:sourceLakeId,potentialLakeBodies:w.lakeBodies,
  ...
  rainfall:climate.rainfall,temperature:climate.temperature,waterModel:'annual-budget-v1',waterBudgetNote:'Synthetic annual precipitation/runoff, evaporation and geology-dependent leakage. Not measured groundwater, seasonal forecasting, or a calibrated climate model.'};
}
