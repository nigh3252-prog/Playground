...
export function createCityView(canvas,{onSelect=()=>{},onChange=()=>{},onSelectSite=()=>{},onDetailRequest=()=>{},autoDetail=true}={}){
 ...
 const cities=new Map(),requested=new Set(),roadBoxes=new WeakMap(),terrainWindows=[];
 let detailLevel='streets';
 ...
 const getView=()=>({kmAcross:city||context?width/scale:0,kmHigh:city||context?height/scale:0,zoom,center:{...center},detailLevel});
 ...
 function requestVisibleDetail(){
  if(!autoDetail||!context||!active)return;
 ...
 function draw(){
  ...
  if(detailLevel!=='parent')for(const patch of terrainWindows){const b=patch.bounds;if(intersects({x:b.x,z:b.z,right:b.x+b.size,bottom:b.z+b.size}))ctx.drawImage(patch.texture,b.x,b.z,b.size,b.size);}
  ...
  const visibleCities=['metro','streets'].includes(detailLevel)?[...cities.values()].filter(c=>intersects({x:c.bounds.x,z:c.bounds.z,right:c.bounds.x+c.bounds.size,bottom:c.bounds.z+c.bounds.size})):[];
  for(const c of visibleCities){
   if(c.bounds.size*scale<35)continue;
   // Metro groups cells into broad named districts. Internal accounting
   // edges are never painted as streets or roof patterns.
   const groups=new Map();
   for(const b of c.blocks){if(!inView(b.center,(c.cellKm||.4)*3))continue;const key=detailLevel==='metro'?b.districtId:b.kind;if(!groups.has(key))groups.set(key,[]);groups.get(key).push(b);}
   for(const [key,blocks]of groups){
    ...
    const color=({residential:'#b7bea3',mixed:'#b8b5a0',downtown:'#b6b09b',industrial:'#9eafb0',park:'#577b56'})[kind];
    ctx.fillStyle=detailLevel==='metro'?tint(color,(Number(key)%5)*.035):color;
    ctx.globalAlpha=kind==='park'?.6:.62;ctx.fill();ctx.globalAlpha=1;
   }
   if(detailLevel==='metro'&&c.bounds.size*scale>160){
    ctx.beginPath();for(const district of c.districts)for(const[a,b]of district.boundary||[]){ctx.moveTo(a.x,a.z);ctx.lineTo(b.x,b.z);}
    ctx.strokeStyle='rgba(72,91,77,.25)';ctx.lineWidth=.55/scale;ctx.stroke();
   }
   for(const building of detailLevel==='streets'?c.buildings||[]:[]){
  ...
  for(const kind of ['local','collector','arterial'])for(const c of visibleCities){
   if(c.bounds.size*scale<35||kind==='local'&&(scale<7||detailLevel!=='streets'))continue;
   for(const road of c.roads){
    if(road.kind!==kind||road.points.length<2)continue;
    if(detailLevel==='metro'&&kind==='collector'&&road.patternId?.startsWith('quarter-'))continue;
  ...
  const district=['metro','streets'].includes(detailLevel)?city?.districts.find(d=>d.id===selected):null;
  if(district){
 ...
 function drawLabels(){
  ...
  if(city&&['metro','streets'].includes(detailLevel)&&city.bounds.size*scale>120){
  ...
  for(const site of [...regionSites()].sort((a,b)=>b.population-a.population)){
   ...
   if(['metro','streets'].includes(detailLevel)&&cities.has(site.id)&&site.radiusKm*scale>22){if(site.id!==city?.siteId)label(site.name,screen(cities.get(site.id).center));continue;}
 ...
 function addCity(value){
  ...
  if(cities.size>(autoDetail?7:64)){
 ...
 function pointerEnd(e){
  ...
  if(tap){
   ...
   if(['metro','streets'].includes(detailLevel))for(const c of [city,...cities.values()].filter(Boolean)){const block=c.blocks.find(b=>inside(point,b.polygon));if(!block)continue;if(c!==city){onSelectSite(c.siteId);return;}selected=block.districtId;invalidate();onSelect(selected);break;}
  }
 ...
 return{
  show(value,options={}){
   if(destroyed)return;clearPointers();cities.clear();requested.clear();terrainWindows.length=0;detailLevel=options.detailLevel||'streets';city=value;context=options.context||null;texture=ctx&&context?context.texture||contextTexture(context):null;selected=null;active=true;zoom=1;
  ...
  removeCity(id){if(city?.siteId!==id)cities.delete(id);invalidate();},
  setDetailLevel(level){detailLevel=level;notify();invalidate();},
  addTerrainWindow(patch){if(!ctx||!patch?.pixels)return;terrainWindows.push({...patch,texture:contextTexture(patch)});if(terrainWindows.length>3)terrainWindows.shift();invalidate();},
  fitParent(){if(context){setViewport({center:{x:context.bounds.x+context.bounds.size/2,z:context.bounds.z+context.bounds.size/2},kmAcross:context.bounds.size*1.06*width/Math.min(width,height)});}},
