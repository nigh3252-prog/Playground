...
export function planParent(seed=431970387,sizeKm=4800){
 return{seed:Number(seed)>>>0,sizeKm,tectonics:planTectonicPlates({seed,sizeKm})};
}
export function generateParentTerrain(options={}){
 ...
 const baseAt=(x,z)=>{const column=clamp(Math.round(x/sizeKm*(n-1)),0,n-1),row=clamp(Math.round(z/sizeKm*(n-1)),0,n-1),value=base.elevation[row*n+column];return{height:value,landBlend:clamp((value+80)/260,0,1)};};
 const geologyPlan=planGeology({seed,sizeKm,northAxis:.2,eastAxis:.8,baseAt}),featurePlan={features:geologyPlan.features};
 for(let i=0;i<N;i++){const raw=base.elevation[i],g=raw>0?applyGeology(featurePlan,mesh.x[i],mesh.z[i],raw):{height:raw,landHistory:0,feature:-1};height[i]=g.height*Number(options.relief||1);landHistory[i]=g.landHistory;featureAt[i]=g.feature;}
 ...
 const terrain={version:'regional-world-v6',stage:1,n,stepKm:mesh.stepKm,config:{seed,n,sizeKm,relief:Number(options.relief||1),rain:Number(options.rain||1),wind:options.wind||'west',source:'generated'},mesh,height,ocean,landHistory,featureAt,features:geologyPlan.features,geology:{features:geologyPlan.features,tectonics:{...plan.tectonics,history},source:'Seeded plate history on a padded parent domain; no child-edge barriers'},erosion:null,parentDomain:{sizeKm,windowKm,seed,nominalSpacingKm:mesh.stepKm,regionalDetail:options.n||193,solverExtent:'entire parent before cropping'}};
