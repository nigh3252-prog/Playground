...
export function blindTerrain(pack,options={}){
 ...
 const raw=generateReferenceTerrain(safe,options),rains=pack.forcing?.stations?.map(s=>Number(s.rainMm)).filter(Number.isFinite)||[],annualRainMm=rains.length?rains.reduce((a,b)=>a+b,0)/rains.length:undefined;
 const w=conditionRegionalDrainage(raw,{protectIntentional:false,annualRainMm});
 return{...w,version:'regional-world-v6',referenceMode:'blind',reference:{...w.reference,measuredInputs:'Raw real DEM + frozen regional climate moisture for terrain-only shallow-sink conditioning; no mapped lake/river constraints or settlement data.',modeledOutputs:'Hydrology and human potential predicted without water/population targets.',edgeWarning:'Cropped region; lake-bottom bathymetry and external upstream inflows are absent. Sub-grid outlet conditioning uses only terrain and climate forcing.'},benchmarkRole:pack.region.role};
}
