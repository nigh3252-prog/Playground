 ...
 * finer resolution. Treating every such raster/mesh pit as a permanent lake
 * creates huge false lakes. This pass breaches only shallow, unprotected
 * depressions using terrain topology alone. It never reads observed lakes,
 * rivers, population, benchmark roles, or place labels.
 */
...
export function conditionRegionalDrainage(terrain,overrides={}){
  ...
  for(let pass=0;pass<cfg.passes;pass++){
    const hydro=generateHydrology({...work,slope:slopeField(mesh,height)}),deepest=new Int32Array(hydro.lakeBodies.length).fill(-1);
    ...
    for(const floorNode of deepest){
      ...
      if(!(fillDepth>.5)||fillDepth>cfg.maxFillDepthM)continue;
  ...
  return{...terrain,height,slope,erosion:{kind:'regional-shallow-breach-v1',breachedBasins,totalCutNodes,maxCutM,maxBreachedFillDepthM,settings:cfg}};
}
